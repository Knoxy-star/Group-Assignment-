...
import zw.ac.uz.dpdms.fire.dto.IncidentCreateRequest;
import zw.ac.uz.dpdms.fire.dto.IncidentResponse;
import zw.ac.uz.dpdms.fire.dto.IncidentUpdateRequest;
...
public class FireService {
    ...
    private static final Hazard SERVICE_HAZARD = Hazard.FIRE;
    ...
    public IncidentResponse create(RequestContext ctx, IncidentCreateRequest req) {
        ...
        incident.setAreaBurnedHectares(req.areaBurnedHectares());
        incident.setSuspectedCause(req.suspectedCause());
        incident.setInjuriesFatalitiesCount(req.injuriesFatalitiesCount());
        incident.setStructuresDestroyedCount(req.structuresDestroyedCount());
        incident.setContained(req.contained());
    ...
    public IncidentResponse getById(RequestContext ctx, Long id) {
        FireIncident incident = findOrThrow(id);
    ...
    public List<IncidentResponse> list(RequestContext ctx, IncidentStatus statusFilter) {
        ...
        if (ctx.isSupervisor()) {
            ...
            List<FireIncident> results = (statusFilter != null)
    ...
    private FireIncident findOrThrow(Long id) {
        return incidentRepository.findById(id)
                .orElseThrow(() -> new NoSuchElementException("Incident not found: " + id));
    }

    private void assertPending(FireIncident incident) {
    ...
    private void writeAudit(Long incidentId, AuditAction action, RequestContext ctx, String notes) {
        FireAuditLog log = new FireAuditLog();
